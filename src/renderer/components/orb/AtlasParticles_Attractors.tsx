...
/**
 * Atlas Desktop - AI Core Particle System with Strange Attractor Morphing
...
interface AtlasParticlesAttractorProps {
  state?: AtlasState;
  audioLevel?: number;
  bass?: number;
  treble?: number;
...
/**
 * Atlas Particles with Attractor Morphing
 */
export function AtlasParticlesAttractors({
  state = 'idle',
  audioLevel = 0,
  bass = 0,
  treble = 0,
  ...
  const previousStateRef = useRef<AtlasState>(state);
  ...
  const uniforms = useMemo(() => {
    console.log('[AtlasParticlesAttractors] Creating uniforms');
    return createShaderUniforms();
  }, []);

  // Create shader material
  const shaderMaterial = useMemo(() => {
    console.log('[AtlasParticlesAttractors] Creating ShaderMaterial');
  ...
  useEffect(() => {
    console.log('[AtlasParticlesAttractors] Initializing with particleCount:', particleCount);
    ...
    console.log('[AtlasParticlesAttractors] Geometry initialized');
  }, [particleCount, state]);
  ...
  useEffect(() => {
    ...
    if (targetAttractor !== currentAttractorRef.current) {
      console.log(
        `[AtlasParticlesAttractors] State change: ${previousStateRef.current} -> ${state}`,
  ...
  useFrame((_, delta) => {
    ...
    if (morphProgressRef.current < 1.0 && targetDataRef.current) {
      ...
      if (morphProgressRef.current >= 1.0) {
        console.log(`[AtlasParticlesAttractors] Morph complete to ${STATE_TO_ATTRACTOR[state]}`);
...
export default AtlasParticlesAttractors;
