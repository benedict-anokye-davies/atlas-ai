...
/**
 * Atlas Desktop - AtlasOrb with Strange Attractor Support
...
import { AtlasParticlesAttractors } from './AtlasParticles_Attractors';
import type { AtlasState } from './attractors';
import type { AudioFeatures } from '../../hooks/useAudioAnalysis';

interface AtlasOrbAttractorProps {
  state?: AtlasState;
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error('[AtlasOrbAttractor] WebGL Error:', error, errorInfo);
  }
...
/**
 * AtlasOrb with Strange Attractor Morphing
 * The AI's living, breathing, morphing presence
 */
export function AtlasOrbAttractor({
  state = 'idle',
  ...
  onStateClick,
  className = '',
}: AtlasOrbAttractorProps) {
  const [isHovered, setIsHovered] = useState(false);

  console.log('[AtlasOrbAttractor] Rendering', {
    state,
    audioFeatures,
    particleCount,
  });
    ...
    >
      <WebGLErrorBoundary>
        <Canvas
          ...
          onCreated={() => {
            console.log('[AtlasOrbAttractor] Canvas created');
          }}
        >
...
export default AtlasOrbAttractor;
